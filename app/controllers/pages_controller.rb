class PagesController < ApplicationController
  def home
  	...
  	if params[:championship]
		...
		@news_next_games = News.where("category = 'Next Games'").where("championship = ?", params[:championship]).order(updated_at: :desc).last(1)
		...
		@news_next_games = News.where("category = 'Next Games'").order(updated_at: :desc).last(1)
